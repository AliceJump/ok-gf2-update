"""Safe OCR character confusion patch (ported from ok-end-field).

Transforms match patterns to accept OCR-confused characters
without modifying OCR output text.

用法:
    - 维护 ``assets/ocr_fix/ocr_text_fix.json``，格式为 ``{错字文本: 正确文本}``。
      例: ``{"鏡像冗余": "镜像冗余"}``
    - 启动时调用 ``install_ocr_text_fix_patch()``（已在 ``src/config.py`` 末尾调用）。
...
_FIX_RELPATH = Path("assets") / "ocr_fix" / "ocr_text_fix.json"

# 模块级缓存：{正确字符: (OCR 可能识别成的错字, ...)}
_CHAR_CONFUSION: dict = {}
...
def _candidate_paths() -> list:
    """返回 ocr_text_fix.json 的候选路径（按优先级）。

    打包后 cwd 不一定是项目根目录，因此额外尝试基于本文件位置的路径。
    """
    paths = [Path.cwd() / _FIX_RELPATH]
    try:
        here = Path(__file__).resolve()
        paths.append(here.parents[2] / _FIX_RELPATH)
    except Exception:
        pass
    return paths


def _load_fix_map() -> dict:
    """读取 ocr_text_fix.json，返回 {错误文本: 正确文本}。"""
    for fix_file in _candidate_paths():
        try:
...
def _build_char_confusion(fix_map: dict) -> dict:
...
def _confused_chars(char: str) -> str:
    """返回该字符的错字集合（已过滤正则元字符），无错字时返回空串。"""
    return "".join(c for c in _CHAR_CONFUSION.get(char, ()) if c not in _REGEX_METACHARS)


def _apply_confusion_to_str(text: str) -> object:
    """对字符串应用字符混淆：笛卡尔积生成所有变体，上限 _MAX_VARIANTS。"""
    char_options = []
    for ch in text:
        if ch in _CHAR_CONFUSION:
            char_options.append((ch,) + tuple(_CHAR_CONFUSION[ch]))
        else:
...
def _apply_confusion_to_regex(pattern):
    ...
    # 快速跳过：没有任何混淆字
    if not any(ch in _CHAR_CONFUSION for ch in raw):
        return pattern
    ...
    for typ, val in tokens:
        if typ == "LITERAL":
            confused = _confused_chars(val)
            if confused:
                result_parts.append("[" + confused + val + "]")
                modified = True
            else:
...
def _apply_confusion_to_match(match):
    ...
    """
    if not _CHAR_CONFUSION:
        return match

    try:
        if isinstance(match, str):
            return _apply_confusion_to_str(match)

        if isinstance(match, re.Pattern):
            return _apply_confusion_to_regex(match)

        if isinstance(match, list):
            result = []
            for item in match:
                transformed = _apply_confusion_to_match(item)
...
def reload_ocr_text_fix() -> dict:
    """重新加载 ocr_text_fix.json 并刷新字符混淆表（测试与热更新用）。"""
    global _CHAR_CONFUSION
    _CHAR_CONFUSION = _build_char_confusion(_load_fix_map())
    return _CHAR_CONFUSION


def install_ocr_text_fix_patch():
    """Patch ``OCR.fix_match_regex``：在既有翻译逻辑之后施加字符混淆。"""
    ...
    except Exception:  # pragma: no cover - ok 不可用时静默跳过
        return
    ...
    def patched_fix_match_regex(self, match):
        ...
        return _apply_confusion_to_match(match)
